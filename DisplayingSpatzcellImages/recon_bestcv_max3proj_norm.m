function recon_bestcv_max3proj_norm(ip, frm, cdir, cnum, cnum_focus, crg)
        ...
        for i = 1:numel(z_range)
            ...
            for ii = 1:N
                if max_cv_z(ii) == i
                    ...
                    img_p = img_p - min(img_p(:));
                    img_p = img_p / max(img_p(:));
                    
                    img_c = img_c - min(img_c(:));
                    img_c = img_c / max(img_c(:));
                    
                    img_f = img_f - min(img_f(:));
