...
% The following block was lifted from segmoviephase:
...
disp('    Press ''G'' to use gfp as image');
disp('    Press ''R'' to use rfp as image');
disp('    Press ''P'' to use phase as image');
disp('    Press  ESC to reclick');
disp('    Press ''Q'' to quit');

% code assumes phase is last
maskcolors = 'YCGRP';
maxmask = 5;
minmask = 1;
mask=5;
...
while i > 0
    ...
    while ~done
        ...
        colorstr = ['-',maskcolors(mask),'-'];
        if (p.numphaseslices > 1) & (upper(maskcolors(mask))=='P'),
            colorstr = [colorstr,'2-'];
        end;
        filename = [p.imageDir p.movieName colorstr mynum '.tif']
        if ~exist(filename)
            disp(['file ' filename ' does not exist! Using YFP.']);
            mask = maxmask;
            continue;
        end
        im = imread(filename);
        if upper(maskcolors(mask))~='P',
            supersize = 2;
            invertit = 0;
            set(mainfig,'Name',['frame ' mynum ' ' maskcolors(mask) 'FP']);
        else
            supersize = 1;
            invertit = 1;
        ...
        title('<space>=OK+save, <esc>=redo, C,Y,G,R,P=switch image, S=skip, Q=quit, Bksp=previm, +/-/= adjust boxsize');
