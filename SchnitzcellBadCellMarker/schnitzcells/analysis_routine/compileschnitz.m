...
if ~existfield(p,'minutesPerFrame')
  p.minutesPerFrame = 10;
end
...
if ~p.quickMode
    ...
    if exist('zzgim')~=1
        disp('zzgim not found, using blank "zzgim".');
        zzgim=[];
    end
    if exist('zzrim')~=1
        disp('zzrim not found, using blank "zzrim".');
        zzrim=[];
    end
    ...
    if exist('GFPmolecules_per_GFLunit')==1
    ...
    if exist('RFPmolecules_per_RFLunit')==1
        p.RFPmolecules_per_RFLunit = RFPmolecules_per_RFLunit;
    else
        disp('No RFP calibration data - using default=1.');
        p.RFPmolecules_per_RFLunit = 1;
    end
    ...
    for i = 1:length(trackRange)
        ...
        rreg=double(rreg);
        if ~isempty(rreg)
            rreg = normalizeF(rreg,rback,zzrim,rect,exptr,p.autoRFL,...
        ...
        %for currSchnitzNum = schnitzesForFrame     % nitzan 2005May16
            ...
            schnitzcells= getcellparams(schnitzcells, rp, Lc, rect, creg,...
                yreg, greg, rreg, exptc, expty, exptg, exptr, currSchnitzNum, age, cellnum); 
            ...
            if (timestamp ~= 'empty')
                ...
                fakeMin    = currFrameNum * p.minutesPerFrame;
...
if length([schnitzcells.FCs])
    ...
    schnitzcells= add_interps(schnitzcells,'FRsmins','FRs','dFRmins','FR_interps_Rdot');
    schnitzcells= add_interps(schnitzcells,'mins','volume','dFRmins','vol_interps_Rdot');
    schnitzcells= add_interps(schnitzcells,'FGsmins','FGs','dFGmins','FG_interps_Gdot');
    schnitzcells= add_interps(schnitzcells,'mins','volume','dFGmins','vol_interps_Gdot');
end
...
schnitzcells= add_interps(schnitzcells,'mins','phase','dFRmins','phase_interps_Rdot');
schnitzcells= add_interps(schnitzcells,'mins','phase','dFGmins','phase_interps_Gdot');
...
function ncells= getcellparams(cells, rp, Lc, rect,...
    creg, yreg, greg, rreg, exptc, expty, exptg, exptr, schnitzno, age, cellnum)
ncells= cells;
% find cell
loc= find(Lc == cellnum);
% extract information
...
if ~isempty(greg),
    ncells(schnitzno).FG(age)= sum(greg(loc));
    ncells(schnitzno).SG(age)= std(greg(loc));
    ncells(schnitzno).MG(age)= ncells(schnitzno).FG(age)/ncells(schnitzno).SZ(age);
    ncells(schnitzno).medG(age) = median(greg(loc));
else
    ncells(schnitzno).FG(age)= NaN;
    ncells(schnitzno).SG(age)= NaN;
    ncells(schnitzno).MG(age)= NaN;
    ncells(schnitzno).medG(age)= NaN;
end;    
if ~isempty(rreg),
    ncells(schnitzno).FR(age)= sum(rreg(loc));
    ncells(schnitzno).SR(age)= std(rreg(loc));
    ncells(schnitzno).MR(age)= ncells(schnitzno).FR(age)/ncells(schnitzno).SZ(age);
    ncells(schnitzno).medR(age) = median(rreg(loc));
else
    ncells(schnitzno).FR(age)= NaN;
    ncells(schnitzno).SR(age)= NaN;
    ncells(schnitzno).MR(age)= NaN;
    ncells(schnitzno).medR(age)= NaN;
end;    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
